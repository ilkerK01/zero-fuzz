...
wired yet — the balance is real and read from chain, the billing is computed from the real
run (see `bill()` in `src/lib/server/scan.ts`), but the debit transaction is not submitted.
This is the top item on the roadmap.

---

## What actually works today

Nothing is claimed here that does not run. This table is the reference for every other
...
**The wallet signs SEP-10, not the server.** The first version had the server hold a secret
and sign the challenge. That authenticates the server, not the user. Moving the signature
client-side through Stellar Wallets Kit means the SEP-10 token belongs to the person whose
contract is being audited. *Trade-off:* one more user interaction, and the demo depends on a
...
**The Windows GNU linker cannot build Soroban `cdylib` test targets.** `cargo test` on the
registry crate fails with `export ordinal too large` on the host. Fixed by running contract
tests inside a `rust:1-slim` container; `stellar contract build` for the wasm itself works
on the host. This is also why the sandbox design — containers all the way down — turned out
to be the only thing that worked on the dev machine.

**Docker bind mounts on Windows reject `/tmp` paths.** Generated tests were being written to
a temp dir the daemon could not see. Fixed by writing into the project tree and passing the
real Windows path with `MSYS_NO_PATHCONV=1`.

**A React state updater read a stale index and crashed the tree.** `setLines(p => [...p, PRE[i]])`
captured `i` late, produced `undefined`, and took the whole page down with
"This page couldn't load". Fixed by capturing the value before the updater runs.

**Turkish locale broke uppercase English text.** With `lang="tr"` on the root element,
CSS `text-transform: uppercase` renders `PAİD` instead of `PAID`. Machine output is English
and monospace, where a dotted capital I is unmissable. Fixed by scoping `lang="en"` to those
sections.

**The model ID moved.** `gemini-2.5-flash-lite` is closed to new API consumers and returns
404 pointing at 3.5. Pinned to `gemini-3.5-flash-lite` via `ZF_MODEL` so it is swappable
